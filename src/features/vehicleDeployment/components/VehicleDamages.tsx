...
export const VehicleDamages: React.FC = () => {
  ...
  // Load real damage data from vehicle service
  useEffect(() => {
    const loadDamageData = async () => {
      try {
        setLoading(true);
        
        // Get deployment history to extract damage information from checklists
        ...
        setDamages(realDamages);
        setFilteredDamages(realDamages);
      } catch (error) {
