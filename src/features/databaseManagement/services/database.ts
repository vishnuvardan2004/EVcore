...
export class DatabaseService {
  ...
  async create<T extends { id: string; createdAt: string; updatedAt: string; createdBy: string }>(
    table: Table<T>,
    ...
    await this.logAction(userId, 'create', table.name, record.id, record);
    return record;
  }
  ...
  // Vehicle operations
  async createVehicle(data: Omit<Vehicle, 'id' | 'createdAt' | 'updatedAt'>, userId: string): Promise<Vehicle> {
